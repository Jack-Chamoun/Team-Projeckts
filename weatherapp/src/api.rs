...
/*put the api_id that you have frfom the mentioned website "openweathermap" ,or just create a .env file and import it from there */
pub async fn fetch_data(city: String) -> Result<Forecast, Box<dyn Error>> {
    let api_id = "your_api_id";
    let url = format!("https://api.openweathermap.org/data/2.5/forecast?q={city}&appid={api_id}&units=metric"); 
...
pub async fn fetch_current_data(currentcity: String) -> Result<CurrentForecast, Box<dyn Error>> {
    let api_id = "your_api_id";
    let url = format!("https://api.openweathermap.org/data/2.5/weather?q={currentcity}&appid={api_id}&units=metric"); 
    let response = Request::get(&url)
        .send()
        .await?;
